...
def partitionfeatures(features, isFull, classId, names, numtestdata, randomPartioning = True):
    ...
    for j in range(numclass):
        for i in range(numtestdata):
            ...
            testnames.extend([names[index] for index in range(len(names)) if cond[index] and isFull[index]])
            testclassid.extend([classId[index] for index in range(len(classId)) if cond[index] and isFull[index]])
